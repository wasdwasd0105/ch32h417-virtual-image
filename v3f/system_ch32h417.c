...
*/
#if !defined(SYSCLK_400M_CoreCLK_V5F_400M_V3F_100M_HSE) && !defined(SYSCLK_480M_CoreCLK_V5F_240M_V3F_120M_HSE) && \
    !defined(SYSCLK_600M_CoreCLK_V5F_300M_V3F_150M_HSE) && !defined(SYSCLK_400M_CoreCLK_V5F_400M_V3F_100M_HSI) && \
    !defined(SYSCLK_480M_CoreCLK_V5F_240M_V3F_120M_HSI) && \
    !defined(SYSCLK_350M_CoreCLK_V5F_350M_V3F_175M_HSE) && !defined(SYSCLK_400M_CoreCLK_V5F_400M_V3F_200M_HSE)
...
#elif defined SYSCLK_600M_CoreCLK_V5F_300M_V3F_150M_HSE
...
#elif defined SYSCLK_350M_CoreCLK_V5F_350M_V3F_175M_HSE
/*********************************************************************
 * SYSCLK 350 MHz (HSE 25 MHz x14), V5F 350 MHz, HCLK 175 MHz -> SDIO_CK 87.5 MHz.
 * Needs LDO_VDDK performance mode (HCLK > 160 MHz).
 */
static void SetSYSCLK_350M_CoreCLK_V5F_350M_V3F_175M_HSE(void)
{
  __IO uint32_t StartUpCounter = 0, HSEStatus = 0, FLASH_Temp = 0;

  /* HCLK above 160 MHz needs "performance mode": raise the digital core LDO
   * (SYS_CFGR0.LDO_VDDK = 101b, ~1.25 V) before the PLL switches, exactly as the
   * vendor's 480/480 config does.  Datasheet: TA <= 70 C, well-cooled, commercial
   * parts only.  The HSE start-up wait below gives the LDO time to settle. */
  {
    vu32 tmp = *(vu32 *)SYS_CFGR0_BASE;
    tmp &= ~(0x7u << 4);
    tmp |= (0x5u << 4);
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* V5F core clock = SYSCLK (350 MHz) */
    RCC->CFGR0 &= (uint32_t)((uint32_t)~(RCC_HPRE));
    RCC->CFGR0 |= (uint32_t)RCC_HPRE_DIV1; 

    /* V3F core clock = HCLK = SYSCLK/2 = 175 MHz -> SDIO_CK 87.5 MHz at CLKDIV 0 */
    RCC->CFGR0 &= (uint32_t)((uint32_t)~(RCC_FPRE));
    RCC->CFGR0 |= (uint32_t)RCC_FPRE_DIV2;  
...
#elif defined SYSCLK_400M_CoreCLK_V5F_400M_V3F_200M_HSE
/*********************************************************************
 * SYSCLK 400 MHz (HSE 25 MHz x16), V5F 400 MHz, HCLK 200 MHz -> SDIO_CK 100 MHz.
 * HCLK 200 MHz is the datasheet maximum, and only with LDO_VDDK performance mode.
 */
static void SetSYSCLK_400M_CoreCLK_V5F_400M_V3F_200M_HSE(void)
{
  __IO uint32_t StartUpCounter = 0, HSEStatus = 0, FLASH_Temp = 0;
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* V5F core clock = SYSCLK (400 MHz) */
    RCC->CFGR0 &= (uint32_t)((uint32_t)~(RCC_HPRE));
    RCC->CFGR0 |= (uint32_t)RCC_HPRE_DIV1; 

    /* V3F core clock = HCLK = SYSCLK/2 = 200 MHz -> SDIO_CK 100 MHz at CLKDIV 0 */
    RCC->CFGR0 &= (uint32_t)((uint32_t)~(RCC_FPRE));
    RCC->CFGR0 |= (uint32_t)RCC_FPRE_DIV2;  
